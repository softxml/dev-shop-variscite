...
//        }
     ...
     * @param $delivery_price_cod
     */
    public function delivery($delivery_id, $delivery_price, $delivery_price_cod)
    {
        $node = new Node('DELIVERY');
        $value = "<DELIVERY_ID>{$delivery_id}</DELIVERY_ID><DELIVERY_PRICE>{$delivery_price}</DELIVERY_PRICE><DELIVERY_PRICE_COD>{$delivery_price_cod}</DELIVERY_PRICE_COD>";
        if (! isset($this->nodes['DELIVERY'])) {
            $this->nodes['DELIVERY'] = array();
        }
        $this->nodes['DELIVERY'][] = $node->value($value);
    }
