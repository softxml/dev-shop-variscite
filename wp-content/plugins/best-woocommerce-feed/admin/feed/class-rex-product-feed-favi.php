    ...
    private function process_attributes_for_param($attributes) {
        foreach ($attributes as $key => $value) {
            if(preg_match('/^Attribute/im', $key)) {
                ...
                $attributes['attributes'][$key] = array(
                    'key'           => $key,
                    'name'          => $value,
                    'value'         => isset($attributes['Attribute_value_'.$param_no]) ? $attributes['Attribute_value_'.$param_no] : '',
                );
            }
        }
        foreach ($attributes as $key => $value) {
            if(preg_match('/^Attribute/im', $key)) {
                $param_no = preg_replace('/[^0-9]/', '', $key);
                unset($attributes['attributes']['Attribute_value_' . $param_no]);
            }
        }
        return $attributes;
    }
