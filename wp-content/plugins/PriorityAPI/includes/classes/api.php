    ...
    {    
        ...
        add_action('init', function(){
            ...
            if($this->get('repost') && wp_verify_nonce($this->get('request'), 'repost')) {
                ...
                $status_error = $response_code >= 200 && $response_code < 300 ? 1 : 0;
                if (!$status_error && $url == 'OREN_ORDERS'){
                    $data = isset($args['body']) ? json_decode($args['body'], true) : '';
                    $order_id = $data && isset($data['WEBNUMBER']) ? $data['WEBNUMBER'] : '';
                    $this->sendEmailError(
                        $this->option('email_error_sync_orders_web'),
                        'Error Sync ' . (is_null( $url ) ? '' : $url) . ($order_id ? ', Order id: ' . $order_id : ''),
                        $response_body
                    );
                }
            }
